...
<template>
  <div :class="['fixed top-0 navbar transition-colors duration-300', { 'navbar-scrolled': isScrolled }, { 'bg-transparent': !isScrolled }]">
    ...
    <div class="flex-none">
      ...
      <div class="dropdown dropdown-end" :class="{ 'dropdown-open': pickerOpen }"
      ...
      <div class="tooltip tooltip-bottom [--tt-bg:var(--color-base-200)] before:text-base-content" :data-tip="$t('component.navbar.calendar.schedule')">
        <nuxt-link-locale to="/dashboard/schedule" class="btn btn-ghost btn-circle">
          <i class="fa-solid fa-calendar"/>
        </nuxt-link-locale>
      </div>

      <div class="tooltip tooltip-bottom [--tt-bg:var(--color-base-200)] before:text-base-content" :data-tip="$t('component.navbar.user.shopping_list')">
